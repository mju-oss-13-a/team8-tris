...
void mainMenuDraw(char *videoBuffer)
{
	...
	for(i = 0; i < VIDEOBUFFER_SIZE; i++)
	{
		...
		{
			if(row >= 2 && column < 22 && row < 24)			
				videoBuffer[i] = mainMenuGridImage[row-2][column];
			else if(row >= 18 && row < 23 && !exitQuest)	
				videoBuffer[i] = ' ';
			else											
	...
	if(!skIntro && mainMenuTotalTimeElapsed < INTR_ANIM_TIME)
	{
		...
		if(mainMenuTotalTimeElapsed >= 0 && mainMenuTotalTimeElapsed < 1000)			//avanzamento stampa del Welcome
			printedChar = (mainMenuTotalTimeElapsed  ) * 36 / 3000;
		else if(mainMenuTotalTimeElapsed >= 1000 && mainMenuTotalTimeElapsed < 2000)	//fermo su welcome
			printedChar = 36;
		else if(mainMenuTotalTimeElapsed >= 2000)										//avanzamento stampa del resto
			printedChar = (mainMenuTotalTimeElapsed - 10000) * (consoleBase*IMAGE_SIZE_H - 36) / 3000 + 36;
