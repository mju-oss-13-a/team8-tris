...
int playTrisUpdate(int deltaT)
{
	...
	if(!enteringAnim) //se ?finita l'introduzione iniziale
	{
		...
		else //se c'?ancora da giocare
		{
			...
			else if(playTrisTotalTimeElapsed - playTrisTimeLastMovement >= IA_LATENCY) // AI턴일때
			{
...
void playTrisDraw(char *videoBuffer)
{
	...
	for(i = 0; i < VIDEOBUFFER_SIZE; i++)
	{
		...
		if(column < IMAGE_SIZE_B && row < IMAGE_SIZE_H)
		{
			...
			else if(row >= 2 && row < 24 && column < 22) //se ?nella griglia stampa la griglia
